...
export default function CompetitorsPage() {
  ...
  async function analyzeSite(url: string): Promise<SiteResult> {
    try {
      ...
      if (!res.ok || !payload.success) {
        return { url, result: null, error: payload.error ?? "Analysis failed" };
      }
  ...
  async function handleCompare() {
    ...
    for (let i = 0; i < urls.length; i++) {
      setLoadingStep("Analyzing site " + (i + 1) + " of " + urls.length + "...");
    ...
    <div className="space-y-4">
      ...
      <Card className="space-y-3 p-4">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <div>
            <label className="text-xs text-zinc-400 mb-1 block">Your Website URL</label>
            <Input value={myUrl} onChange={(e) => setMyUrl(e.target.value)} placeholder="https://yoursite.com" />
          </div>
          <div>
            <label className="text-xs text-zinc-400 mb-1 block">Target Keyword</label>
            <Input value={keyword} onChange={(e) => setKeyword(e.target.value)} placeholder="e.g. CRM for sales teams" />
          </div>
          <div>
            <label className="text-xs text-zinc-400 mb-1 block">Industry</label>
            <select value={industry} onChange={(e) => setIndustry(e.target.value as typeof industry)} className="h-11 w-full rounded-xl border border-input bg-card px-4 text-sm">
        ...
        <Button onClick={handleCompare} disabled={loading || !myUrl || !keyword || !competitor1}>
          {loading ? loadingStep : "Compare Now"}
        </Button>
      </Card>
      ...
      {results.length > 0 && (
        <div className="space-y-4">
          <div className="grid gap-3" style={{ gridTemplateColumns: "repeat(" + results.length + ", 1fr)" }}>
            {results.map((r, i) => (
              <Card key={r.url} className={"p-4 space-y-2 border " + (r.result ? scoreBorder(r.result.geo_score) : "border-zinc-800")}>
                ...
                {r.result ? (
                  <>
                    <div className={"text-4xl font-bold " + scoreColor(r.result.geo_score)}>{r.result.geo_score}</div>
                    <p className="text-xs text-zinc-400">AI Citation: <span className="capitalize text-zinc-200">{r.result.ai_citation_probability}</span></p>
                    <p className="text-xs text-zinc-400 line-clamp-2">{r.result.visibility_summary}</p>
                  </>
                ) : (
          ...
          {myResult && myResult.result && competitorResults.some((r) => r.result) && (
            <div className="grid gap-4 md:grid-cols-2">
              <Card className="p-4 border-emerald-900/30">
                <h3 className="font-medium text-emerald-400 mb-3">Where You Win</h3>
                <ul className="space-y-2">
                  {myResult.result.strengths.map((s) => (
                    <li key={s} className="text-sm text-zinc-300 flex gap-2"><span className="text-emerald-400">-</span>{s}</li>
                  ))}
                </ul>
              </Card>
              <Card className="p-4 border-red-900/30">
                <h3 className="font-medium text-red-400 mb-3">Where They Win</h3>
                <ul className="space-y-2">
                  {myResult.result.critical_gaps.map((g) => (
                    <li key={g} className="text-sm text-zinc-300 flex gap-2"><span className="text-red-400">-</span>{g}</li>
                  ))}
                </ul>
              </Card>
            </div>
          ...
          {myResult && myResult.result && myResult.result.quick_wins.length > 0 && (
            <Card className="p-4">
              <h3 className="font-medium mb-3">Quick Wins to Beat Competitors</h3>
              <ul className="space-y-2">
                {myResult.result.quick_wins.map((w) => (
                  <li key={w} className="text-sm text-zinc-300 flex gap-2"><span className="text-amber-400">-</span>{w}</li>
                ))}
              </ul>
            </Card>
          )}

          {competitorResults[0] && competitorResults[0].result && (
            <Card className="p-4 flex items-center justify-between">
              <div>
                <p className="font-medium">Ready to outreach?</p>
                <p className="text-sm text-zinc-400">Generate personalized outreach based on this analysis.</p>
              </div>
              <Button onClick={() => {
                const pains = myResult && myResult.result ? myResult.result.critical_gaps.join("; ") : "";
                const params = new URLSearchParams({ company: competitor1, pains: pains, angle: "They outrank you in AI search for " + keyword });
